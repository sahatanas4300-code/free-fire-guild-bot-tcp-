...
from Crypto.Util.Padding import pad, unpad
# =================================================================
# 🕸️ SPIDER GLORY BOOSTER ENGINE (By Anas)
# =================================================================
...
async def anas_spider_glory_booster():
    ...
    print(f"\n🚀 [SPIDER AUTO-START] REAL Glory Farm Activated!")
    try:
    ...
    while glory_running:
        ...
        for acc_uid, password in accounts_data.items():
            ...
            try:
                ...
                print(f"❌ Error with UID {acc_uid}: {e}") # এই লাইনটি যোগ করুন
...
def start_insta_api():
    ...
    print(f"ðŸš€ Starting Insta API on port {port}")
...
class MultiAccountManager:
    ...
    def load_accounts(self):
        ...
        try:
            ...
            print(f"âŒ Accounts file {self.accounts_file} not found!")
            return {}
        except Exception as e:
            print(f"âŒ Error loading accounts: {e}")
            return {}
    ...
    async def get_account_token(self, uid, password):
        ...
        try:
            ...
            print(f"âŒ Error getting token for {uid}: {e}")
            return None, None
    ...
    async def send_join_from_account(self, target_uid, account_uid, password, key, iv, region):
        ...
        try:
            ...
            print(f"âŒ Error sending join from {account_uid}: {e}")
            return False
...
async def RedZed_SendInv(bot_uid, uid, key, iv):
    ...
    try:
        ...
        print(f"âŒ Error in RedZed_SendInv: {e}")
        import traceback
        traceback.print_exc()
        return None
...
async def auto_start_loop(team_code, uid, chat_id, chat_type, key, iv, region):
    ...
    while not stop_auto:
        try:
            # Send status message
            status_msg = f"[B][C][FFA500]ðŸ¤– Auto Start Bot\nðŸŽ¯ Team: {team_code}\nâš¡ Joining team..."
            ...
            start_msg = f"[B][C][00FF00]âœ… Joined team {team_code}\nðŸŽ¯ Starting match for {start_spam_duration} seconds..."
            ...
            # Wait after match
            wait_msg = f"[B][C][FFFF00]â³ Match started! Bot in lobby waiting {wait_after_match} seconds..."
            ...
            # Leave squad
            leave_msg = f"[B][C][FF0000]ðŸ”„ Leaving team {team_code} to rejoin and start again..."
            ...
            error_msg = f"[B][C][FF0000]âŒ Auto start error: {str(e)}\n"
...
async def reset_bot_state(key, iv, region):
    ...
    try:
        ...
        print(f"âŒ Error resetting bot: {e}")
        return False    
        ...
        for account_uid, password in accounts_data.items():
            try:
                ...
                if join_packet:
                    await SEndPacKeT(whisper_writer, online_writer, 'OnLine', join_packet)
                    success_count += 1
                    print(f"âœ… Join sent from authenticated account: {account_uid}")
                
                # Important: Wait between requests
                await asyncio.sleep(2)
                
            except Exception as e:
                print(f"âŒ Error with account {account_uid}: {e}")
                continue
        
        return success_count, total_accounts
        
    except Exception as e:
        print(f"âŒ Multi-account join error: {e}")
        return 0, 0
...
async def handle_badge_command(cmd, inPuTMsG, uid, chat_id, key, iv, region, chat_type):
    ...
    if len(parts) < 2:
        error_msg = f"[B][C][FF0000]âŒ Usage: /{cmd} (uid)\nExample: /{cmd} 123456789\n"
        await safe_send_message(chat_type, error_msg, uid, chat_id, key, iv)
        return
    ...
    if not target_uid.isdigit():
        error_msg = f"[B][C][FF0000]âŒ Please write a valid player ID!\n"
        await safe_send_message(chat_type, error_msg, uid, chat_id, key, iv)
        return
    
    # Send initial message
    initial_msg = f"[B][C][1E90FF]ðŸŒ€ Request received! Preparing to spam {target_uid}...\n"
    ...
    try:
        ...
        error_msg = f"[B][C][FF0000]âŒ Error in /{cmd}: {str(e)}\n"
        await safe_send_message(chat_type, error_msg, uid, chat_id, key, iv)
...
async def create_authenticated_join(target_uid, account_uid, key, iv, region):
    ...
    try:
        ...
        print(f"âŒ Error creating join packet: {e}")
        return None        
    ...
    async def create_account_join_packet(self, target_uid, account_uid, open_id, access_token, key, iv, region):
        ...
        try:
            ...
            print(f"âŒ Error creating join packet for {account_uid}: {e}")
            return None
...
async def auto_rings_emote_dual(sender_uid, key, iv, region):
    ...
    try:
        ...
        print(f"ðŸ¤– Bot performed dual Rings emote with sender {sender_uid} and bot {bot_uid}!")
...
async def handle_reject_completion(spam_task, target_uid, sender_uid, chat_id, chat_type, key, iv):
    ...
    try:
        ...
        if spam_count >= 150:
            completion_msg = f"[B][C][00FF00]âœ… Reject Spam Completed Successfully for ID {target_uid}\nâœ… Total packets sent: {spam_count * 2}\n"
        else:
            completion_msg = f"[B][C][FFFF00]âš ï¸ Reject Spam Partially Completed for ID {target_uid}\nâš ï¸ Total packets sent: {spam_count * 2}\n"
        ...
        error_msg = f"[B][C][FF0000]âŒ ERROR in reject spam: {str(e)}\n"
...
async def lag_team_loop(team_code, key, iv, region):
    ...
    start_time = time.time() # à¦¸à¦®à§Ÿ à¦—à¦£à¦¨à¦¾ à¦¶à§à¦°à§

    

    while lag_running:

        # à§¨à§¦ à¦¸à§‡à¦•à§‡à¦¨à§à¦¡ à¦¹à§Ÿà§‡ à¦—à§‡à¦²à§‡ à¦²à§à¦ª à¦¬à¦¨à§à¦§ à¦¹à¦¬à§‡
...
[C][B][11EAFD]â€Žâ”â”â”â”â”â”â”â”â”â”â”â”
[C][B][FFB300]TELEGRAM: [FFFFFF]@SubhajitMajiRobot [00FF00]!!
"""
...
def send_insta_info(username):
    try:
        ...
        if response.status_code != 200:
            return f"[B][C][FF0000]âŒ Instagram API Error! Status Code: {response.status_code}"
        ...
        return f"""
[B][C][FB0364]â•­[D21A92]â”€[BC26AB]â•®[FFFF00]â•”â•â•â•â•â•â•â•â•—
[C][B][FF7244]â”‚[FE4250]â—¯[C81F9C]Ö¯â”‚[FFFF00]â•‘[FFFFFF]INSTAGRAM_INFO[FFFF00]â•‘
[C][B][FDC92B]â•°[FF7640]â”€[F5066B]â•¯[FFFF00]â•šâ•â•â•â•â•â•â•â•
[C][B][FFFF00]â”â”â”â”â”â”â”â”â”â”â”â”
...
"""
        ...
        return "[B][C][FF0000]âŒ Instagram API Connection Failed!"
    except Exception as e:
        return f"[B][C][FF0000]âŒ Unexpected Error: {str(e)}"
...
async def ultra_quick_emote_attack(team_code, emote_id, target_uid, key, iv, region):
    ...
    try:
        ...
        join_packet = await GenJoinSquadsPacket(team_code, key, iv)
        await SEndPacKeT(whisper_writer, online_writer, 'OnLine', join_packet)
        print(f"ðŸ¤– Joined team: {team_code}")
        ...
        print(f"ðŸŽ­ Performed emote {emote_id} to UID {target_uid}")
        ...
        leave_packet = await ExiT(None, key, iv)
        await SEndPacKeT(whisper_writer, online_writer, 'OnLine', leave_packet)
        print(f"ðŸšª Left team: {team_code}")
...
async def spam_request_loop_with_cosmetics(target_uid, key, iv, region):
    ...
    while spam_request_running and count < max_requests:
        try:
            ...
            count += 1
            print(f"âœ… Sent cosmetic invite #{count} to {target_uid} with badge {current_badge}")
        ...
        try:
            ...
            while True:
                            ...
                            if '2' in packet_json and 'data' in packet_json['2']:
                                ...
                                if ('1' in emote_data and '2' in emote_data and 
                                    ...
                                    if '1' in nested and '3' in nested:
                                        ...
                                        print(f"ðŸŽ¯ EMOTE HIJACK DETECTED!")
                                        print(f"ðŸ‘¤ Sender: {sender_uid}")
                                        print(f"ðŸŽ­ Original emote: {emote_id}")
                                        ...
                                        print(f"ðŸŽ Sent special emote 909038002 to {sender_uid}")
                                        ...
                                        try:
                                            ...
                                            print(f"ðŸ”„ Mirroring user's emote {emote_id} back")
                                        except ValueError:
                                            print(f"âŒ Could not convert emote ID: {emote_id}")
                                        ...
                                        try:
                                            ...
                                            print(f"ðŸ¤– Bot also doing emote {emote_id}")
                                        except Exception as e:
                                            print(f"âŒ Bot self-emote failed: {e}")
                                        
                                        continue  # Skip other processing for this packet
                                        
                    except Exception as e:
                        print(f"âŒ Emote hijack error: {e}")
                        pass
                ...
                if data_hex.startswith('0f00') and len(data_hex) > 100:
                    print(f"ðŸ“¡ Received status response packet")
    
                    try:
                        if '08' in data_hex:
                            proto_part = f'08{data_hex.split("08", 1)[1]}'
                        else:
                            print("âš ï¸ Status packet structure missing '08' marker.")
                            continue
                        ...
                        if parsed_data:
                            ...
                            if "2" in parsed_json and parsed_json["2"]["data"] == 15:
                                ...
                                print(f"âœ… Parsed status for {player_id}: {player_status}")
                                ...
                                try:
                                    ...
                                    if ("5" in StatusData and "data" in StatusData["5"] and 
                                        ...
                                        print(f"ðŸŽ¯ SPECIAL CONDITION MET: Player {player_id} is in SOLO mode with special flag 11=1")
                                        cache_entry['special_state'] = 'SOLO_WITH_FLAG_1'
                
                                except Exception as cond_error:
                                    print(f"âš ï¸ Error checking special condition: {cond_error}")
                                ...
                                if "IN ROOM" in player_status:
                                    try:
                                        ...
                                        if room_id:
                                            ...
                                            print(f"ðŸ  Room ID extracted: {room_id}")
                                ...
                                # Extract leader if in squad
                                    ...
                                    try:
                                        ...
                                        if leader_id:
                                            ...
                                            print(f"ðŸ‘‘ Leader ID: {leader_id}")
                                ...
                                print(f"âœ… Status cache updated: {player_id} = {player_status}")
                
                    except Exception as e:
                        print(f"âŒ Error parsing status: {e}")
                        import traceback
                        traceback.print_exc()
...
"""
                                ...
                                error_msg = f"[B][C][FF0000]âŒ ERROR! Please provide a question after /ai\nExample: /ai What is Free Fire?\n"
                        ...
                        if inPuTMsG.strip().startswith('/likes '):
                            ...
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                error_msg = f"[B][C][FF0000]âŒ ERROR! Usage: /likes (uid)\nExample: /likes 123456789\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                target_uid = parts[1]
                        ...
                        if inPuTMsG.strip().startswith('/ms '):
                            ...
                            try:
                                ...
                                error_msg = f"[B][C][FF0000]âŒ ERROR! Something went wrong:\n{str(e)}"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                        ...
                        if inPuTMsG.strip().startswith('/gali '):
                            ...
                            try:
                                ...
                                if len(parts) < 2:
                                    ...
                                    messages = [
                                        "{Name} TÆRI SÆXY BHEN KI CHXT ME ME L0DA DAAL KAR RAAT BHAR JOR JOR SE CH0DUNGA",
                                        "{Name} MADHERXHOD TÆRI MÃÃ KI KALI G4ND MÆ LÃ€ND MARU",
                                        "{Name} TÆRI BHÆN KI TIGHT CHXT KO 5G KI SPEED SE CHÃ’D DU",
                                        "{Name} TÆRI BEHEN KI CHXT ME L4ND MARU",
                                        "{Name} TÆRI MÃÃ KI CHXT 360 BAR",
                                        "{Name} TÆRI BÆHÆN KI CHXT 720 BAR",
                                        "{Name} BEHEN KE L0DE",
                                        "{Name} MADARCHXD",
                                        "{Name} BETE TÆRA BAAP HUN ME",
                                        "{Name} G4NDU APNE BAAP KO H8 DEGA",
                                        "{Name} KI MÃ€Ã€ KI CHXT PER NIGHT 4000",
                                        "{Name} KI BÆHÆN KI CHXT PER NIGHT 8000",
                                        "{Name} R4NDI KE BACHHÆ APNE BAP KO H8 DEGA",
                                        "INDIA KA NO-1 G4NDU {Name}",
                                        "{Name} CHAPAL CH0R",
                                        "{Name} TÆRI MÃ€Ã€ KO GB ROAD PE BETHA KE CHXDUNGA",
                        ...
                        if inPuTMsG.strip().startswith('/ig '):
                            ...
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                error_msg = f"[B][C][FF0000]âŒ ERROR! Usage: /ig <username>\nExample: /ig virat.kohli\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
        ...
        # Use ThreadPoolExecutor to avoid blocking the async loop
                        ...
                        if inPuTMsG.strip().startswith('/bio '):
                            ...
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                error_msg = f"[B][C][FF0000]âŒ ERROR! Usage: /bio <uid>\nExample: /bio 4368569733\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                target_uid = parts[1]
                        ...
                        if inPuTMsG.strip().startswith('/quick'):
                            ...
                            if len(parts) < 3:
                                ...
                                initial_message = f"[B][C][FFFF00]âš¡ QUICK EMOTE ATTACK!\n\n[FFFFFF]ðŸŽ¯ Team: [00FF00]{team_code}\n[FFFFFF]ðŸŽ­ Emote: [00FF00]{emote_id}\n[FFFFFF]ðŸ‘¤ Target: [00FF00]{target_name}\n[FFFFFF]â±ï¸ Estimated: [00FF00]2 seconds\n\n[FFFF00]Executing sequence...\n"
                                await safe_send_message(response.Data.chat_type, initial_message, uid, chat_id, key, iv)
                                ...
                                try:
                                    ...
                                    if success:
                                        success_message = f"[B][C][00FF00]âœ… QUICK ATTACK SUCCESS!\n\n[FFFFFF]ðŸ·ï¸ Team: [00FF00]{team_code}\n[FFFFFF]ðŸŽ­ Emote: [00FF00]{emote_id}\n[FFFFFF]ðŸ‘¤ Target: [00FF00]{target_name}\n\n[00FF00]Bot joined â†’ emoted â†’ left! âœ…\n"
                                    else:
                                        success_message = f"[B][C][FF0000]âŒ Regular attack failed: {result}\n"
                        ...
                        if inPuTMsG.strip().startswith('/inv '):
                            ...
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                error_msg = f"[B][C][FF0000]âŒ ERROR! Usage: /inv (uid)\nExample: /inv 123456789\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                target_uid = parts[1]
                                ...
                                try:
                                    ...
                                    error_msg = f"[B][C][FF0000]âŒ ERROR sending invite: {str(e)}\n"
                                    await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                        ...
                        if inPuTMsG.strip().startswith('/roommsg'):
                            ...
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 3:
                                error_msg = f"[B][C][FF0000]âŒ Usage: /roommsg (room_id) (message)\nExample: /roommsg 489775386 Hello room!\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                room_id = parts[1]
                                message = " ".join(parts[2:])
        
                                initial_msg = f"[B][C][00FF00]ðŸ“¢ Sending to room {room_id}: {message}\n"
                                ...
                                try:
                                    ...
                                    success_msg = f"[B][C][00FF00]âœ… Message sent to room {room_id}!\n"
                                    await safe_send_message(response.Data.chat_type, success_msg, uid, chat_id, key, iv)
                                    print(f"âœ… Room message sent to {room_id}: {message}")
            
                                except Exception as e:
                                    error_msg = f"[B][C][FF0000]âŒ Failed: {str(e)}\n"
                                    await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                        ...
                        if inPuTMsG.strip() == "/admin":
                            ...
                            admin_message = """
[C][B][FF0000]â•”â•â•â•â•â•â•â•â•â•â•â•—
[FFFFFF]âœ¨ SUBSCRIBE IN YOUTUBE   
[FFFFFF]          âš¡ SECRETXMODSâ¤ï¸  
[FFFFFF]                   thank for support 
[FF0000]â• â•â•â•â•â•â•â•â•â•â•â•£
[FFD700]âš¡ OWNER : [FFFFFF]SM    
[FFD700]âœ¨ Name on telegram : [FFFFFF] @SubhajitMajiRobotâ¤ï¸  
[FF0000]â•šâ•â•â•â•â•â•â•â•â•â•â•
[FFD700]âœ¨ Developer â€”ÍžÍŸÍž </> SM â„ï¸  âš¡
"""
                        ...
                        if inPuTMsG.strip().startswith('/multijoin'):
                            ...
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                error_msg = f"[B][C][FF0000]âŒ Usage: /multijoin (target_uid)\nExample: /multijoin 123456789\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                target_uid = parts[1]
                                ...
                                initial_msg = f"[B][C][00FF00]ðŸš€ Starting multi-join attack on {target_uid}...\n"
                                await safe_send_message(response.Data.chat_type, initial_msg, uid, chat_id, key, iv)
                                ...
                                try:
                                    ...
                                    if success_count > 0:
                                        result_msg = f"""
[B][C][00FF00]âœ… MULTI-JOIN ATTACK COMPLETED!

ðŸŽ¯ Target: {target_uid}
âœ… Successful Requests: {success_count}
ðŸ“Š Total Attempts: {total_attempts}
âš¡ Different squad variations sent!

ðŸ’¡ Check your game for join requests!
"""
                                    else:
                                        result_msg = f"[B][C][FF0000]âŒ All join requests failed! Check bot connection.\n"
            
                                    await safe_send_message(response.Data.chat_type, result_msg, uid, chat_id, key, iv)
            
                                except Exception as e:
                                    error_msg = f"[B][C][FF0000]âŒ Multi-join error: {str(e)}\n"
                                    await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                        ...
                        if inPuTMsG.strip().startswith('/fastmultijoin'):
                            ...
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                error_msg = f"[B][C][FF0000]âŒ ERROR! Usage: /fastmultijoin (uid)\nExample: /fastmultijoin 123456789\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                target_uid = parts[1]
                                ...
                                accounts_data = load_accounts()
                                if not accounts_data:
                                    error_msg = f"[B][C][FF0000]âŒ ERROR! No accounts found!\n"
                                    await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                    return
                                
                                initial_msg = f"[B][C][00FF00]âš¡ FAST MULTI-ACCOUNT JOIN SPAM!\nðŸŽ¯ Target: {target_uid}\nðŸ‘¥ Accounts: {len(accounts_data)}\n"
                                await safe_send_message(response.Data.chat_type, initial_msg, uid, chat_id, key, iv)
                                ...
                                try:
                                    ...
                                    for uid, password in accounts_data.items():
                                        try:
                                            ...
                                            await SEndPacKeT(whisper_writer, online_writer, 'OnLine', join_packet)
                                            join_count += 1
                                            print(f"âœ… Fast join from account {uid}")
                    
                                            # Very short delay
                                            await asyncio.sleep(0.1)
                    
                                        except Exception as e:
                                            print(f"âŒ Fast join failed for {uid}: {e}")
                                            continue
            
                                    success_msg = f"[B][C][00FF00]âœ… FAST MULTI-JOIN COMPLETED!\nðŸŽ¯ Target: {target_uid}\nâœ… Successful: {join_count}/{len(accounts_data)}\nâš¡ Speed: Ultra fast\n"
                                    await safe_send_message(response.Data.chat_type, success_msg, uid, chat_id, key, iv)
            
                                except Exception as e:
                                    error_msg = f"[B][C][FF0000]âŒ ERROR in fast multi-join: {str(e)}\n"
                                    await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                        ...
                        if inPuTMsG.strip().startswith('/reject'):
                            ...
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                error_msg = f"[B][C][FF0000]âŒ ERROR! Usage: /reject (target_uid)\nExample: /reject 123456789\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                target_uid = parts[1]
                                ...
                                # Send start message
                                start_msg = f"[B][C][1E90FF]ðŸŒ€ Started Reject Spam on: {target_uid}\nðŸŒ€ Packets: 150 each type\nðŸŒ€ Interval: 0.2 seconds\n"
                        ...
                        if inPuTMsG.strip() == '/reject_stop':
                            if reject_spam_task and not reject_spam_task.done():
                                reject_spam_running = False
                                reject_spam_task.cancel()
                                stop_msg = f"[B][C][00FF00]âœ… Reject spam stopped successfully!\n"
                                await safe_send_message(response.Data.chat_type, stop_msg, uid, chat_id, key, iv)
                            else:
                                error_msg = f"[B][C][FF0000]âŒ No active reject spam to stop!\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                        ...
                        if inPuTMsG.strip().startswith('/room'):
                            ...
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                error_msg = f"[B][C][FF0000]âŒ ERROR! Usage: /room (uid)\nExample: /room 123456789\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                target_uid = parts[1]
                                room_id = parts[2]
                                ...
                                if not target_uid.isdigit():
                                    error_msg = f"[B][C][FF0000]âŒ ERROR! Please write a valid player ID!\n"
                                    await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                    return
        
                                # Send initial message
                                initial_msg = f"[B][C][00FF00]ðŸ” Working on room spam for {target_uid}...\n"
                                await safe_send_message(response.Data.chat_type, initial_msg, uid, chat_id, key, iv)
                                ...
                                try:
                                    ...
                                    room_msg = f"[B][C][00FF00]ðŸŽ¯ Detected player in room {room_id}\n"
                                    ...
                                    start_msg = f"[B][C][00FF00]ðŸš€ Starting spam: {spam_count} packets to room {room_id}\n"
                                    await safe_send_message(response.Data.chat_type, start_msg, uid, chat_id, key, iv)
                                    ...
                                    for i in range(spam_count):
                                        ...
                                        # Progress updates
                                        if (i + 1) % 25 == 0:
                                            progress_msg = f"[B][C][00FF00]ðŸ“¦ Progress: {i+1}/{spam_count} packets sent\n"
                                    ...
                                    success_msg = f"[B][C][00FF00]âœ… ROOM SPAM COMPLETED!\nðŸŽ¯ Target: {target_uid}\nðŸ“¦ Packets: {spam_count}\nðŸ  Room: {room_id}\nâš¡ Speed: Ultra fast\n"
                                    await safe_send_message(response.Data.chat_type, success_msg, uid, chat_id, key, iv)
                                    print(f"Room spam completed for UID: {target_uid}")
            
                                except Exception as e:
                                    error_msg = f"[B][C][FF0000]âŒ ERROR in room spam: {str(e)}\n"
                                    await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                        ...
                        if inPuTMsG.strip().startswith('/spam'):
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                error_msg = "[B][C][FF0000]âŒ Usage: /spam <uid>\nExample: /spam 123456789\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                target_uid = parts[1]
                                ...
                                initial_msg = f"[B][C][1E90FF]ðŸŒ€ Request received! Preparing to spam {target_uid} with all badges...\n"
                                await safe_send_message(response.Data.chat_type, initial_msg, uid, chat_id, key, iv)
                        ...
                        if inPuTMsG.strip().startswith('/joinroom'):
                            ...
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 3:
                                error_msg = f"[B][C][FF0000]âŒ Usage: /joinroom (room_id) (password)\nExample: /joinroom 123456 0000\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                room_id = parts[1]
                                room_password = parts[2]
        
                                initial_msg = f"[B][C][00FF00]ðŸš€ Joining custom room...\nðŸ  Room: {room_id}\nðŸ”‘ Password: {room_password}\n"
                                await safe_send_message(response.Data.chat_type, initial_msg, uid, chat_id, key, iv)
                                ...
                                try:
                                    ...
                                    success_msg = f"[B][C][00FF00]âœ… Joined custom room {room_id}!\nðŸ¤– Bot is now in room chat!\n"
                                    await safe_send_message(response.Data.chat_type, success_msg, uid, chat_id, key, iv)
            
                                except Exception as e:
                                    error_msg = f"[B][C][FF0000]âŒ Failed to join room: {str(e)}\n"
                                    await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                        ...
                        if inPuTMsG.strip().startswith('/createroom'):
                            ...
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 3:
                                error_msg = f"[B][C][FF0000]âŒ Usage: /createroom (room_name) (password) [players=4]\nExample: /createroom BOTROOM 0000 4\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                room_name = parts[1]
                                room_password = parts[2]
                                ...
                                initial_msg = f"[B][C][00FF00]ðŸ  Creating custom room...\nðŸ“› Name: {room_name}\nðŸ”‘ Password: {room_password}\nðŸ‘¥ Max Players: {max_players}\n"
                                await safe_send_message(response.Data.chat_type, initial_msg, uid, chat_id, key, iv)
                                ...
                                try:
                                    ...
                                    success_msg = f"[B][C][00FF00]âœ… Custom room created!\nðŸ  Room: {room_name}\nðŸ”‘ Password: {room_password}\nðŸ‘¥ Max: {max_players}\nðŸ¤– Bot is now hosting!\n"
                                    await safe_send_message(response.Data.chat_type, success_msg, uid, chat_id, key, iv)
            
                                except Exception as e:
                                    error_msg = f"[B][C][FF0000]âŒ Failed to create room: {str(e)}\n"
                                    await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)                                                                                                                                                                                                               
                        ...
                        if inPuTMsG.startswith('/join'):
                            ...
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                error_msg = f"[B][C][FF0000]âŒ ERROR! Usage: /join (team_code)\nExample: /join ABC123\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                CodE = parts[1]
                                ...
                                try:
                                    ...
                                    # SUCCESS MESSAGE
                                    success_message = f"[B][C][00FF00]âœ… SUCCESS! Joined squad: {CodE}!\nðŸ’ Dual Rings emote activated!\nðŸ¤– Bot + You = ðŸ’•\n"
                                    await safe_send_message(response.Data.chat_type, success_message, uid, chat_id, key, iv)
                                    ...
                                    try:
                                        ...
                                        if ghost_packet:
                                            ...
                                            success_message = f"[B][C][00FF00]âœ… SUCCESS! Ghost joined squad: {CodE}!\nðŸ’ Dual Rings emote activated!\nðŸ¤– Bot + You = ðŸ’•\n"
                                            await safe_send_message(response.Data.chat_type, success_message, uid, chat_id, key, iv)
                                        else:
                                            error_msg = f"[B][C][FF0000]âŒ ERROR! Failed to create ghost join packet.\n"
                                            await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                    
                                    except Exception as ghost_error:
                                        print(f"Ghost join also failed: {ghost_error}")
                                        error_msg = f"[B][C][FF0000]âŒ ERROR! Failed to join squad: {str(ghost_error)}\n"
                                        await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                        ...
                        if inPuTMsG.strip().startswith('/ghost'):
                            ...
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                error_msg = f"[B][C][FF0000]âŒ ERROR! Usage: /ghost (team_code)\nExample: /ghost ABC123\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                CodE = parts[1]
                                ...
                                try:
                                    ...
                                    error_msg = f"[B][C][FF0000]âŒ ERROR! Ghost join failed: {str(e)}\n"
                                    await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                        ...
                        if inPuTMsG.strip().startswith('/lag '):
                            ...
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                error_msg = f"[B][C][FF0000]âŒ ERROR! Usage: /lag (team_code)\nExample: /lag ABC123\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                team_code = parts[1]
                        ...
                        if inPuTMsG.strip() == '/stop lag':
                            if lag_task and not lag_task.done():
                                ...
                                error_msg = f"[B][C][FF0000]âŒ ERROR! No active lag attack to stop!\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                        ...
                        if inPuTMsG.strip().startswith('/title'):
                            ...
                            try:
                                ...
                                # SUCCESS MESSAGE
                                success_message = f"[B][C][00FF00]âœ… SUCCESS! Title sent to current team!\n"
                                await safe_send_message(response.Data.chat_type, success_message, uid, chat_id, key, iv)
        
                            except Exception as e:
                                print(f"Title send failed: {e}")
                                error_msg = f"[B][C][FF0000]âŒ ERROR! Failed to send title: {str(e)}\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                        ...
                        if inPuTMsG.strip().startswith('/e'):
                            ...
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 3:
                                error_msg = f"[B][C][FF0000]âŒ ERROR! Usage: /e (uid) (emote_id)\nExample: /e 123456789 909000001\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                continue
                            ...
                            if not s:
                                try:
                                    ...
                                    error_msg = f"[B][C][FF0000]âŒ ERROR sending emote: {str(e)}\n"
                                    await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                error_msg = f"[B][C][FF0000]âŒ ERROR! Invalid UID format. Usage: /e (uid) (emote_id)\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                        ...
                        if inPuTMsG.strip().startswith('/lw'):
                            ...
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                error_msg = f"[B][C][FF0000]âŒ ERROR! Usage: /lw (team_code)\nExample: /lw 123456\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                team_code = parts[1]
                                ...
                                if not team_code.isdigit():
                                    error_msg = f"[B][C][FF0000]âŒ ERROR! Team code must be numbers only!\nExample: /lw 123456\n"
                                    await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                    continue
                                
                                # Check if already running
                                if auto_start_running:
                                    error_msg = f"[B][C][FF0000]âŒ ERROR! Auto start already running for team {auto_start_teamcode}!\nUse /stop_auto to stop first.\n"
                                    await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                    continue
                                ...
                                # Send initial message
                                initial_msg = f"""
[B][C][00FFFF]ðŸ¤– AUTO START BOT ACTIVATED!

ðŸŽ¯ Team Code: {team_code}
âš¡ Action: Join â†’ Start â†’ Wait â†’ Leave â†’ Repeat
â° Start Spam: {start_spam_duration} seconds
â³ Wait Time: {wait_after_match} seconds
ðŸ”„ Loop: Continuous 24x7

ðŸ’¡ To stop: /stop_auto
"""
                                await safe_send_message(response.Data.chat_type, initial_msg, uid, chat_id, key, iv)
                        ...
                        if inPuTMsG.strip() == '/stop_auto':
                            ...
                            if auto_start_task and not auto_start_task.done():
                                stop_auto = True # à¦²à§à¦ª à¦¥à¦¾à¦®à¦¾à¦¨à§‹à¦° à¦¸à¦¿à¦—à¦¨à§à¦¯à¦¾à¦²
                                auto_start_running = False
                                auto_start_task.cancel() # à¦Ÿà¦¾à¦¸à§à¦•à¦Ÿà¦¿ à¦«à§‹à¦°à§à¦¸ à¦•à¦°à§‡ à¦¬à¦¨à§à¦§ à¦•à¦°à¦¾
                                success_msg = f"[B][C][00FF00]âœ… SUCCESS! Auto start bot stopped successfully!\n"
                                await safe_send_message(response.Data.chat_type, success_msg, uid, chat_id, key, iv)
                            else:
                                error_msg = f"[B][C][FF0000]âŒ ERROR! No active auto start to stop!\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                        ...
                        if inPuTMsG.strip().startswith('/random'):
                            ...
                            # SUCCESS MESSAGE
                            if len(uids) == 1:
                                success_msg = f"[B][C][00FF00]âœ… SUCCESS! Evolution emote cycle started!\nðŸŽ¯ Target: Yourself\nðŸŽ­ Emotes: All 18 evolution emotes\nâ° Delay: 5 seconds between emotes\nðŸ”„ Cycle: Continuous loop until /sevos\n"
                            else:
                                success_msg = f"[B][C][00FF00]âœ… SUCCESS! Evolution emote cycle started!\nðŸŽ¯ Targets: Yourself + {len(uids)-1} other players\nðŸŽ­ Emotes: All 18 evolution emotes\nâ° Delay: 5 seconds between emotes\nðŸ”„ Cycle: Continuous loop until /sevos\n"
                        ...
                        if inPuTMsG.strip() == '/sevos':
                            if evo_cycle_task and not evo_cycle_task.done():
                                evo_cycle_running = False
                                evo_cycle_task.cancel()
                                success_msg = f"[B][C][00FF00]âœ… SUCCESS! Evolution emote cycle stopped successfully!\n"
                                await safe_send_message(response.Data.chat_type, success_msg, uid, chat_id, key, iv)
                                print("Evolution emote cycle stopped by command")
                            else:
                                error_msg = f"[B][C][FF0000]âŒ ERROR! No active evolution emote cycle to stop!\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                        ...
                        if inPuTMsG.strip().startswith('/fast'):
                            ...
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 3:
                                error_msg = f"[B][C][FF0000]âŒ ERROR! Usage: /fast uid1 [uid2] [uid3] [uid4] emoteid\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                ...
                                if not uids or not emote_id:
                                    error_msg = f"[B][C][FF0000]âŒ ERROR! Invalid format! Usage: /fast uid1 [uid2] [uid3] [uid4] emoteid\n"
                                    await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                else:
                        ...
                        if inPuTMsG.strip().startswith('/p'):
                            ...
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 4:
                                error_msg = f"[B][C][FF0000]âŒ ERROR! Usage: /p (uid) (emote_id) (times)\nExample: /p 123456789 909000001 10\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                try:
                                    target_uid = parts[1]
                                    ...
                                    if times <= 0:
                                        error_msg = f"[B][C][FF0000]âŒ ERROR! Times must be greater than 0!\n"
                                        await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                    elif times > 100:
                                        error_msg = f"[B][C][FF0000]âŒ ERROR! Maximum 100 times allowed for safety!\n"
                                        await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                    else:
                                    ...
                                    error_msg = f"[B][C][FF0000]âŒ ERROR! Invalid number format! Usage: /p (uid) (emote_id) (times)\n"
                                    await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                except Exception as e:
                                    error_msg = f"[B][C][FF0000]âŒ ERROR! {str(e)}\n"
                                    await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                        ...
                        if inPuTMsG.strip().startswith('/spm_inv'):
                            ...
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                error_msg = f"[B][C][FF0000]âŒ Usage: /spm_inv (uid)\nExample: /spm_inv 123456789\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                target_uid = parts[1]
                                ...
                                # SUCCESS MESSAGE
                                success_msg = f"[B][C][00FF00]âœ… COSMETIC SPAM STARTED!\nðŸŽ¯ Target: {target_uid}\nðŸ“¦ Requests: 30\nðŸŽ­ Features: V-Badges + Cosmetics\nâš¡ Each invite has different cosmetics!\n"
                                await safe_send_message(response.Data.chat_type, success_msg, uid, chat_id, key, iv)
                        ...
                        if inPuTMsG.strip() == '/stop spm_inv':
                            if spam_request_task and not spam_request_task.done():
                                ...
                                error_msg = f"[B][C][FF0000]âŒ ERROR! No active spam request to stop!\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                        ...
                        if inPuTMsG.strip().startswith('/evo '):
                            ...
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                error_msg = f"[B][C][FF0000]âŒ ERROR! Usage: /evo uid1 [uid2] [uid3] [uid4] number(1-21)\nExample: /evo 123456789 1\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                ...
                                if not uids or not number:
                                    error_msg = f"[B][C][FF0000]âŒ ERROR! Invalid format! Usage: /evo uid1 [uid2] [uid3] [uid4] number(1-21)\n"
                                    await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                else:
                                    try:
                                        number_int = int(number)
                                        if number_int not in EMOTE_MAP:
                                            error_msg = f"[B][C][FF0000]âŒ ERROR! Number must be between 1-21 only!\n"
                                            await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                        else:
                                            ...
                                            if success:
                                                ...
                                                error_msg = f"[B][C][FF0000]âŒ ERROR! {result_msg}\n"
                                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                            
                                    except ValueError:
                                        error_msg = f"[B][C][FF0000]âŒ ERROR! Invalid number format! Use 1-21 only.\n"
                                        await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                        ...
                        if inPuTMsG.strip().startswith('/evo_fast '):
                            ...
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                error_msg = f"[B][C][FF0000]âŒ ERROR! Usage: /evo_fast uid1 [uid2] [uid3] [uid4] number(1-21)\nExample: /evo_fast 123456789 1\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                # Parse uids and number
                                uids = []
                                ...
                                if not uids or not number:
                                    error_msg = f"[B][C][FF0000]âŒ ERROR! Invalid format! Usage: /evo_fast uid1 [uid2] [uid3] [uid4] number(1-21)\n"
                                    await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                else:
                                    try:
                                        number_int = int(number)
                        ...
                        if inPuTMsG.strip().startswith('/evo_c '):
                            ...
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 3:
                                error_msg = f"[B][C][FF0000]âŒ ERROR! Usage: /evo_c uid1 [uid2] [uid3] [uid4] number(1-21) time(1-100)\nExample: /evo_c 123456789 1 10\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                ...
                                if not uids or not number or not time_val:
                                    error_msg = f"[B][C][FF0000]âŒ ERROR! Invalid format! Usage: /evo_c uid1 [uid2] [uid3] [uid4] number(1-21) time(1-100)\n"
                                    await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                else:
                                    try:
                                        number_int = int(number)
                                        ...
                                        if number_int not in EMOTE_MAP:
                                            ...
                                            error_msg = f"[B][C][FF0000]âŒ ERROR! Time must be between 1-100 only!\n"
                                            await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                        else:
                                        ...
                                        error_msg = f"[B][C][FF0000]âŒ ERROR! Invalid number/time format! Use numbers only.\n"
                                        await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                        ...
                        if inPuTMsG.strip() == '/stop evo_fast':
                            if evo_fast_spam_task and not evo_fast_spam_task.done():
                                ...
                                error_msg = f"[B][C][FF0000]âŒ ERROR! No active evolution fast spam to stop!\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                        ...
                        if inPuTMsG.strip() == '/stop evo_c':
                            if evo_custom_spam_task and not evo_custom_spam_task.done():
                                ...
                                error_msg = f"[B][C][FF0000]âŒ ERROR! No active evolution custom spam to stop!\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)

# IMPROVED TREE-STYLE HELP MENU SYSTEM (Commands in their original menus) ðŸŒ³
                        if inPuTMsG.strip().lower() in ("help", "/help", "menu", "/menu", "commands"):
                            ...
                            # Header
                            header = f"[b][c]{get_random_color()}Hey User Welcome To S M Ë£ RoÊ™á´á´›"
                            await safe_send_message(response.Data.chat_type, header, uid, chat_id, key, iv)
                            await asyncio.sleep(0.2)

                            # â”€â”€â”€â”€â”€ Group Commands â”€â”€â”€â”€â”€
                            group_commands = """[C][B][FFD700]â•â•â•âš¡ GROUP COMMANDS âš¡â•â•â•[00FFFF][B]
â”œâ”€ [00FFFF]Create 3-Player Group
â”‚  â””â”€ [FF69B4]/3
â”œâ”€ [00FFFF]Create 5-Player Group
â”‚  â””â”€ [FF69B4]/5
â”œâ”€ [00FFFF]Create 6-Player Group
â”‚  â””â”€ [FF69B4]/6
â”œâ”€ [00FFFF]Invite Player
â”‚  â””â”€ [FF69B4]/inv [uid]
â”œâ”€ [00FFFF]Join Team
â”‚  â””â”€ [FF69B4]/join [team_code]
â”œâ”€ [00FFFF]Leave Group
â”‚  â””â”€ [FF69B4]/exit
â””â”€ [00FFFF]Start Match
   â””â”€ [FF69B4]/start
[00FFFF]â”â”â”â”â”â”â”â”â”â”â”â”[FF69B4]"""
                            await safe_send_message(response.Data.chat_type, group_commands, uid, chat_id, key, iv)
                            await asyncio.sleep(0.2)

                            # â”€â”€â”€â”€â”€ Advanced Commands â”€â”€â”€â”€â”€
                            advanced_commands = """[C][B][800080]â•â•â•âš¡ ADVANCED COMMANDS âš¡â•â•â•[FF1493][B]
â”œâ”€ [FF1493]Spam Invites (30x)
â”‚  â””â”€ [BA55D3]/spm_inv [uid]
â”œâ”€ [FF1493]Stop Spam Invites
â”‚  â””â”€ [BA55D3]/stop spm_inv
â”œâ”€ [FF1493]Ghost Join Team
â”‚  â””â”€ [BA55D3]/ghost [code]
â”œâ”€ [FF1493]Lag Attack Team
â”‚  â””â”€ [BA55D3]/lag [code]
â”œâ”€ [FF1493]Stop Lag Attack
â”‚  â””â”€ [BA55D3]/stop lag
â””â”€ [FF1493]Reject Spam
   â””â”€ [BA55D3]/reject [uid]
[FF1493]â”â”â”â”â”â”â”â”â”â”â”â”[BA55D3]"""
                            await safe_send_message(response.Data.chat_type, advanced_commands, uid, chat_id, key, iv)
                            await asyncio.sleep(0.2)

                            # â”€â”€â”€â”€â”€ Emote Commands â”€â”€â”€â”€â”€
                            emote_commands = """[C][B][32CD32]â•â•â•âš¡ EMOTE COMMANDS âš¡â•â•â•[7CFC00][B]
â”œâ”€ [7CFC00]Send Single Emote
â”‚  â””â”€ [32CD32]/e [uid] [id]
â”œâ”€ [7CFC00]Fast Emote (25x)
â”‚  â””â”€ [32CD32]/fast [uid] [id]
â””â”€ [7CFC00]Custom Emote (X times)
   â””â”€ [32CD32]/p [uid] [id] [x]
[7CFC00]â”â”â”â”â”â”â”â”â”â”â”â”[32CD32]"""
                            await safe_send_message(response.Data.chat_type, emote_commands, uid, chat_id, key, iv)
                            await asyncio.sleep(0.2)

                            # â”€â”€â”€â”€â”€ Evolution Emote Commands â”€â”€â”€â”€â”€
                            evo_commands = """[C][B][FFA500]â•â•â•âš¡ EVOLUTION EMOTES âš¡â•â•â•[FF6347][B]
â”œâ”€ [FF6347]Send Evolution Emote
â”‚  â””â”€ [FFA500]/evo [uid] [1-21]
â”œâ”€ [FF6347]Fast Evo (25x)
â”‚  â””â”€ [FFA500]/evo_fast [uid] [1-21]
â”œâ”€ [FF6347]Custom Evo (X times)
â”‚  â””â”€ [FFA500]/evo_c [uid] [1-21] [x]
â”œâ”€ [FF6347]Auto Cycle All Evo Emotes
â”‚  â””â”€ [FFA500]/random [uid]
â””â”€ [FF6347]Stop Evo Emote Cycle
   â””â”€ [FFA500]/sevos
[FF6347]â”â”â”â”â”â”â”â”â”â”â”â”[FFA500]"""
                            await safe_send_message(response.Data.chat_type, evo_commands, uid, chat_id, key, iv)
                            await asyncio.sleep(0.2)

                            # â”€â”€â”€â”€â”€ AI & Utility Commands â”€â”€â”€â”€â”€
                            ai_commands = """[C][B][1E90FF]â•â•â•âš¡ TOOLS & FUN COMMANDS âš¡â•â•â•[00CED1][B]
â”œâ”€ [00CED1]Get player bio by uid
â”‚  â””â”€ [1E90FF]/bio [uid]
â”œâ”€ [00CED1]Fetch Instagram User Info
â”‚  â””â”€ [1E90FF]/ig [username]
â”œâ”€ [00CED1]Send custom spam message
â”‚  â””â”€ [1E90FF]/ms <text>
â”œâ”€ [00CED1]Ask AI Anything
â”‚  â””â”€ [1E90FF]/ai [question]
â”œâ”€ [00CED1]Admin Information
â”‚  â””â”€ [1E90FF]/admin
â””â”€ [00CED1]Bot Status Check
   â””â”€ [1E90FF]/status
[00CED1]â”â”â”â”â”â”â”â”â”â”â”â”[1E90FF]"""
                            await safe_send_message(response.Data.chat_type, ai_commands, uid, chat_id, key, iv)
                            await asyncio.sleep(0.2)

                            # â”€â”€â”€â”€â”€ Badges Commands â”€â”€â”€â”€â”€
                            badge_commands = """[C][B][FF4500]â•â•â•âš¡ BADGE JOIN REQUESTS âš¡â•â•â•[FF69B4][B]
â”œâ”€ [FF69B4]Join Req Craftland Badge
â”‚  â””â”€ [FF4500]/s1 [uid]
â”œâ”€ [FF69B4]Join Req New V-Badge
â”‚  â””â”€ [FF4500]/s2 [uid]
â”œâ”€ [FF69B4]Join Req Moderator Badge
â”‚  â””â”€ [FF4500]/s3 [uid]
â”œâ”€ [FF69B4]Join Req Small V-Badge
â”‚  â””â”€ [FF4500]/s4 [uid]
â”œâ”€ [FF69B4]Join Req Pro Badge
â”‚  â””â”€ [FF4500]/s5 [uid]
â””â”€ [FF69B4]Join Requests All Badge
   â””â”€ [FF4500]/spam [uid]
[FF69B4]â”â”â”â”â”â”â”â”â”â”â”â”[FF4500]"""
                            await safe_send_message(response.Data.chat_type, badge_commands, uid, chat_id, key, iv)
                            await asyncio.sleep(0.2)

                            
                            footer ="""[00FFFA]â•”â•â€¢â•â•â€¢â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â€¢â•â•â€¢â•â•—
[FF1493]â•‘ âš¡ [B][FFFF00]ROBOT INFO[FFFF00][/B] âš¡
[00FFFA]â•‘
[FFFF00]â•‘ ðŸ‘¤ Developer    ::[FF1493] Anas (SPIDER)
[32CD32]â•‘ ðŸ’» Status        :: [32CD32]ONLINE
[1E90FF]â•‘ ðŸ›  Version      :: [1E90FF]ALL IN ONE
[00FFFA]â•šâ•â€¢â•â•â€¢â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â€¢â•â•â€¢â•â•"""
...
async def MaiiiinE():
    ...
    os.system('clear')
    print("ðŸ”„ Starting TCP Connections...")
    print("ðŸ“¡ Connecting to Free Fire servers...")
    print("ðŸŒ Server connection established")
    ...
    ready_event = asyncio.Event()
        # স্পাইডার গ্লোরি বুস্টার অটো স্টার্ট 
    ...
    print("â”‚ â–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘ â”‚")
    print("â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜")
    time.sleep(0.5)
    os.system('clear')
    print("Connecting to Free Fire servers...")
    print("â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”")
    print("â”‚ â–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘ â”‚")
    print("â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜")
    time.sleep(0.5)
    os.system('clear')

    print("ðŸ¤– BLACK_APIS BOT - ONLINE")
    print("â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”")
    print("â”‚ â–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆ â”‚")
    print("â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜")
    print(f"ðŸ”¹ UID: {TarGeT}")
    print(f"ðŸ”¹ Name: {acc_name}")
    print(f"ðŸ”¹ Status: ðŸŸ¢ READY")
    print("")
    ...
    os.system('clear')
    print(render('SM', colors=['white', 'green'], align='center'))
    print('')
    print("ðŸ¤– SUBHAJIT MAJI ROBOT - ONLINE")
    print(f"ðŸ”¹ UID: {TarGeT}")
    print(f"ðŸ”¹ Name: {acc_name}")
    print(f"ðŸ”¹ Status: ðŸŸ¢ READY")
...
def handle_keyboard_interrupt(signum, frame):
    ...
    print("ðŸ‘‹ Thanks for using SM ROBOT")
    sys.exit(0)
